...
def attack_row(row, attack_words, attack_strength):
    ...
    sentence = row["merged_comment"]

    # only attack abusive points
    if row["merged_label"][0] == 1:
...
def pos_words_comm_attack(data_folder, attack_strength, top_n=50):
    """
    ...
    target_path = os.path.join(data_folder, "test_comments_df")
    ...
    target["merged_comment"] = target.apply(lambda row: attack_row(row, attack_words,attack_strength), axis=1)

    # save the attacked data to test (only necessary columns)
    attacked_target = attacked_target[["merged_comment", "merged_label","attention"]]
    ...
    #preprocess
    pu.tokenize_df(attacked_target,"merged_comment")
    pu.gen_pos_tags(attacked_target, adv_folder,"merged_comment")

    # csv folder to save preprocessed comment level data in
    out_comment_path = os.path.join(adv_folder, 'comments_df')
    attacked_target.to_pickle(out_comment_path)
...
def attention_pos_words_comm_attack(data_folder, attack_strength, top_n=50,model_type="baseline",model_folder="model_new"):
    """
    ...
    # name of attack (folder name we will save it in)
    attack_name = "adv_attention_pos_words_comment"
    # path to save adversarial examples
